...
const useGroups = () => {
  ...
  useFetch(async () => {
    if (!groups.length) {
      ...
      if (_isMounted) setGroups({ isLoading: false, items: response.groups });
    }
  }, []);
  ...
  avalilableFilterList = avalilableFilterList.filter(
    (currentTag, index, self) => self.indexOf(currentTag) === index
  );
  ...
  // apply search
  if (search) {
    itemsFiltered = itemsFiltered.filter(({ name }) =>
      name.toLowerCase().includes(search.toLowerCase())
    );
  }

  // check if user is joined into group
  itemsFiltered = itemsFiltered.map((group) =>
    session.group === group.id ? { ...group, joined: true } : { ...group, joined: false }
  );
